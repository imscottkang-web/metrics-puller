...
from metrics_lib.reach_archive import gather_reach_rows, read_archived_reach_rows
...
def test_gather_reach_rows_downloads_and_returns_rows_on_first_run(tmp_path):
    ...
    rows = gather_reach_rows(client, archive_dir, JOB_NAME)
...
def test_gather_reach_rows_writes_day_file_and_ledger(tmp_path):
    ...
    gather_reach_rows(client, archive_dir, JOB_NAME)
...
def test_gather_reach_rows_does_not_redownload_or_double_count_on_second_run(tmp_path):
    ...
    rows1 = gather_reach_rows(client1, archive_dir, JOB_NAME)

    client2 = FakeReportingClient("job-abc", [report], {url: text})
    rows2 = gather_reach_rows(client2, archive_dir, JOB_NAME)
...
def test_gather_reach_rows_does_not_redownload_when_create_time_is_older_or_equal(tmp_path):
    ...
    gather_reach_rows(client1, archive_dir, JOB_NAME)
    ...
    rows = gather_reach_rows(client2, archive_dir, JOB_NAME)
...
def test_gather_reach_rows_does_not_archive_a_bad_body_and_retries_next_run(tmp_path):
    ...
    printed_bad = []
    with pytest.raises(MetricsApiError):
        gather_reach_rows(client_bad, archive_dir, JOB_NAME, printer=printed_bad.append)
    ...
    rows = gather_reach_rows(client_good, archive_dir, JOB_NAME, printer=printed_good.append)
...
def test_gather_reach_rows_treats_non_dict_ledger_entries_as_absent(tmp_path):
    ...
    rows = gather_reach_rows(client, archive_dir, JOB_NAME, printer=lambda _msg: None)
...
def test_gather_reach_rows_prints_first_ingest_diagnostic_when_ledger_starts_empty(tmp_path):
    ...
    gather_reach_rows(client, archive_dir, JOB_NAME, printer=printed.append)
...
def test_gather_reach_rows_no_diagnostic_on_a_later_run(tmp_path):
    ...
    gather_reach_rows(client1, archive_dir, JOB_NAME, printer=lambda _msg: None)
    ...
    gather_reach_rows(client2, archive_dir, JOB_NAME, printer=printed.append)
...
# --- read_archived_reach_rows: the network-free half of the channel-identity guard ---
# (metrics.py's _check_channel_identity uses this so its cross-check still works when
# the live API cannot be reached at all)


def test_read_archived_reach_rows_reads_local_files_only_no_client_needed(tmp_path):
    archive_dir = tmp_path / "_reach_reports"
    archive_dir.mkdir(parents=True)
    (archive_dir / "20260701.csv").write_text(
        CSV_HEADER + ",channel_id\n2026-07-01,vid001,1000,0.048,UC_house_channel\n",
        encoding="utf-8",
    )

    rows = read_archived_reach_rows(archive_dir)

    assert rows == [
        {"date": "2026-07-01", "video_id": "vid001", "impressions": 1000, "ctr": 4.8,
         "channel_id": "UC_house_channel"}
    ]


def test_read_archived_reach_rows_on_missing_directory_returns_empty(tmp_path):
    # A brand-new house with no reach reports archived yet - must not crash the
    # channel-identity check; there is simply nothing yet to cross-check against.
    assert read_archived_reach_rows(tmp_path / "never_created") == []
